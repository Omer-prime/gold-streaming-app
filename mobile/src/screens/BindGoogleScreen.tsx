...
type Nav = NativeStackNavigationProp<ProfileStackParamList, "BindGoogle">;



export default function BindGoogleScreen() {
  const navigation = useNavigation<Nav>();
  const route = useRoute<any>();

  const provider = t("bindSocial.providers.google");
  ...
  const load = async (id: string) => {
    try {
      ...
      setCurrent(st.googleId ?? null);
    } catch (e: any) {
      Alert.alert(t("common.error"), e?.message || t("bindSocial.alerts.loadFailed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const onBind = async () => {
    ...
    try {
      setSaving(true);
      await bindSocial(userId, "google", val);
  ...
  const onUnbind = async () => {
    ...
    try {
      setSaving(true);
      await unbindSocial(userId, "google");
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={["top"]}>
      <View className="flex-row items-center px-4 pt-3 pb-2 border-b border-gray-100">
        ...
        <Text className="text-[18px] font-semibold text-[#111827]">{t("bindSocial.titles.google")}</Text>
      </View>
      ...
      <View className="px-6 pt-6">
        {loading ? (
          <View className="py-10 items-center">
            <ActivityIndicator />
            <Text className="mt-3 text-[12px] text-[#6B7280]">{t("common.loadingText")}</Text>
          </View>
        ) : (
          <>
            ...
            <View className="rounded-2xl border border-gray-200 px-4 py-3">
              <TextInput
                ...
                placeholder={t("bindSocial.placeholders.google")}
            ...
            <Pressable
              ...
              className={`mt-4 h-11 rounded-full items-center justify-center ${
                !userId || !providerId.trim() || saving ? "bg-[#E5E7EB]" : "bg-[#6366F1]"
              }`}
            >
            ...
            {current ? (
              <Pressable
                ...
                className="mt-4 h-11 rounded-full items-center justify-center bg-[#111827]"
              >
