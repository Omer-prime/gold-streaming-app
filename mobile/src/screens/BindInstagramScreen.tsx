...
type Nav = NativeStackNavigationProp<ProfileStackParamList, "BindInstagram">;



export default function BindInstagramScreen() {
  const navigation = useNavigation<Nav>();
  const route = useRoute<any>();

  const provider = t("bindSocial.providers.instagram");
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setCurrent(st.instagramId ?? null);
      } catch {
        // ignore
      }
    })();
  ...
  const onBind = async () => {
    ...
    try {
      setSaving(true);
      await bindSocial(userId, "instagram", val);
  ...
  const onUnbind = async () => {
    ...
    try {
      setSaving(true);
      await unbindSocial(userId, "instagram");
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={["top"]}>
      <View className="flex-row items-center px-4 pt-3 pb-2 border-b border-gray-100">
        ...
        <Text className="text-[18px] font-semibold text-[#111827]">{t("bindSocial.titles.instagram")}</Text>
      </View>
      ...
      <View className="px-6 pt-6">
        ...
        <View className="rounded-2xl border border-gray-200 px-4 py-3">
          <TextInput
            ...
            placeholder={t("bindSocial.placeholders.instagram")}
