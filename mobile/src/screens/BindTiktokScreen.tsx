...
type Nav = NativeStackNavigationProp<ProfileStackParamList, "BindTiktok">;



export default function BindTiktokScreen() {
  const navigation = useNavigation<Nav>();
  const route = useRoute<any>();

  const provider = t("bindSocial.providers.tiktok");
  ...
  const onBind = async () => {
    ...
    try {
      setSaving(true);
      await bindSocial(userId, "tiktok", val);
  ...
  const onUnbind = async () => {
    ...
    try {
      setSaving(true);
      await unbindSocial(userId, "tiktok");
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={["top"]}>
      <View className="flex-row items-center px-4 pt-3 pb-2 border-b border-gray-100">
        ...
        <Text className="text-[18px] font-semibold text-[#111827]">{t("bindSocial.titles.tiktok")}</Text>
      </View>
      ...
      <View className="px-6 pt-6">
        ...
        <View className="rounded-2xl border border-gray-200 px-4 py-3">
          <TextInput
            ...
            placeholder={t("bindSocial.placeholders.tiktok")}
