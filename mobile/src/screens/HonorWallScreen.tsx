  ...
  const loadHonor = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/profile/honor?userId=${encodeURIComponent(
          userId
        )}`
      );
    ...
    <SafeAreaView className="flex-1 bg-[#F9FAFB]" edges={["top"]}>
      ...
      <View className="flex-row px-4 mt-1">
        <Pressable onPress={() => navigation.navigate("MyProfile")}>
          <Text className="text-[14px] text-[#9CA3AF] mr-6">Data</Text>
        </Pressable>
        <View>
...
const HonorCard: React.FC<HonorCardProps> = ({
  title,
  count,
  fromColor,
  toColor,
  ...
  <View className="mb-4">
    <View className="flex-row items-center justify-between mb-1 px-1">
      ...
      <Text className="text-[11px] text-[#6B7280]">{count} &gt;</Text>
    </View>
    <LinearGradient
